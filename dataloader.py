...
class dataloader():
    ...
    def get_PAE_inputs(self, nonimg):
        ...
        # aff_adj, feature_sim = Reader.get_static_affinity_adj(self.node_ftr, self.pd_dict)
        aff_adj = Reader.get_static_affinity_adj(self.node_ftr, self.pd_dict)
        flatten_ind = 0
        for i in range(n):
            for j in range(i+1, n):
                edge_index[:,flatten_ind] = [i,j]   # 边索引 2x378885
                edgenet_input[flatten_ind]  = np.concatenate((nonimg[i], nonimg[j]))  # 两个点的nonimage数据 378885x6
                aff_score[flatten_ind] = aff_adj[i][j]  # 边的数据 378885
                flatten_ind +=1

        assert flatten_ind == num_edge, "Error in computing edge input"
        
        # keep_ind = np.where(aff_score > 1.1)[0]
        # keep_ind = np.where(aff_score > 0.85)[0]
