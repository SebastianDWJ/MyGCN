...
def fetch_filenames(subject_IDs, file_type):
    ...
    for i in range(len(subject_IDs)):
        ...
        os.chdir('E:\\postgraduate\\ASD\\EV_GCN-master\\data\\ABIDE_pcp\\cpac\\filt_noglobal')
        try:
...
def get_ids(num_subjects=None):
    ...
    subject_IDs = np.genfromtxt(os.path.join('./data/ABIDE_pcp/cpac/filt_noglobal', 'subject_IDs.txt'), dtype=str)
...
def get_static_affinity_adj(features, pd_dict):
    # pd_affinity = create_affinity_graph_from_scores(['SITE_ID'], pd_dict)
    ...
    # pd_affinity = create_affinity_graph_from_scores(['SEX', 'SITE_ID','AGE_AT_SCAN'], pd_dict)
